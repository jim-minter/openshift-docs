...
be overridden via the `OPENSHIFT_JENKINS_JVM_ARCH` environment variable.

By default the Jenkins JVM uses 50% of the container memory limit for its heap.
This value can be modified via the `CONTAINER_HEAP_PERCENT` environment
...
container, e.g. shell scripts or `oc` commands run locally from pipelines, are
...
per-slave image basis via the Jenkins configuration.  The memory request
